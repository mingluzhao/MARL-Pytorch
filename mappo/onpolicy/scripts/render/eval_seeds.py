...
# calculated_data = {
# 3pred_1prey-0block-sheepspeed0.7-indivd0-discreteFalse-seed1
# mean: 68.3, se: 1.9521917507199307
#
# 3pred_1prey-0block-sheepspeed0.7-indivd0-discreteFalse-seed2
# mean: 192.95, se: 6.5487945857475
# 3pred_2prey-0block-sheepspeed0.7-indivd0-discreteFalse-seed1
# mean: 53.0, se: 4.158694127019353
# 3pred_2prey-0block-sheepspeed0.7-indivd0-discreteFalse-seed2
# mean: 184.85, se: 1.1568446195078277
#
# }
...
def evaluateWolfSheepIterativeTrain(df):
    ...
    exp_name = f'../results/MPE/{all_args.scenario_name}/rmappo/{numWolves}pred_{numSheep}prey-{numBlocks}block-sheepspeed{trainSheepSpeed}-indivd{individualRewardWolf}-discrete{all_args.discrete_action}-seed{seed}'
    exp_name_dir = Path(os.path.split(os.path.dirname(os.path.abspath(__file__)))[0] + "/results/" + f'MPE/{all_args.scenario_name}/rmappo/{numWolves}pred_{numSheep}prey-{numBlocks}block-sheepspeed{trainSheepSpeed}-indivd{individualRewardWolf}-discrete{all_args.discrete_action}-seed{seed}')

    # print(exp_name_dir)
    if not exp_name_dir.exists():
        # print(f'No file for {numWolves}pred_{numSheep}prey-{numBlocks}block-sheepspeed{trainSheepSpeed}-indivd{individualRewardWolf}-discrete{all_args.discrete_action}-seed{seed}')
        return pd.Series({'mean': -1, 'se': -1})
...
def main():
    ...
    independentVariables['seed'] = list(range(30))
    ...
    resultLoc = os.path.join(dirName, 'contin_eval_output_0108_seeds.pkl')
