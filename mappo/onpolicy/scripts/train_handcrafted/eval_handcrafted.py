...
def evaluateWolfSheepIterativeTrain(df):
    ...
    pattern = f'../results/MPE/{all_args.scenario_name}/rmappo/{numWolves}pred_{numSheep}prey-{numBlocks}block-sheepspeed{trainSheepSpeed}-indivd{individualRewardWolf}-discrete{all_args.discrete_action}-seed{seed}'
    exp_name = pattern
    ...
    for episode in range(numEpisodes):
        ...
        for step in range(maxTimeStep):
            ...
            if individualRewardWolf:
                ...
                num_kill = np.sum(rewards[0][:numWolves] // killReward)
            else:
                group_reward = rewards[0][0]
                num_kill = group_reward // killReward

            episode_reward_list.append(group_reward)
            episode_kill_list.append(num_kill)
            state = nextState

        rewards_list.append(np.sum(episode_reward_list))
        kills_list.append(np.sum(episode_kill_list))

    meanTrajReward = np.mean(rewards_list)
    seTrajReward = np.std(rewards_list) / np.sqrt(len(rewards_list) - 1)

    meanKill = np.mean(kills_list)
    seKill = np.std(kills_list) / np.sqrt(len(kills_list) - 1)

    print(exp_name)
    print(f'mean: {meanTrajReward}, se: {seTrajReward}, meankill: {meanKill}')

    return pd.Series({'mean': meanTrajReward, 'se': seTrajReward, 'meanKill': meanKill, 'seKill': seKill})


def main():

    independentVariables = OrderedDict()
    independentVariables['evalSheepSpeed'] = [1.5]#, 1.1, 1.5]
    independentVariables['trainSheepSpeed'] = [1.1]
    independentVariables['individualRewardWolf'] = [0, 1]
    independentVariables['numSheep'] = [1, 2, 4]
    independentVariables['seed'] = list(range(10, 30))
    ...
    resultLoc = os.path.join(dirName, 'continuous_eval_output_0125_speed1.5.pkl')
    ...
    # plt.show()
    plt.savefig(os.path.join(dirName, 'continuous_eval_output_0125.png'))
