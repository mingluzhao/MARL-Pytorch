...
def run(config):
    ...
    fileName = "model{}predators{}cost{}speed{}selfish".format(config.num_predators, config.cost, config.speed,
                                                               config.selfish)
    ...
    print(
        "train: {} predators, {} prey, {} blocks, {} episodes with {} steps each eps, preySpeed: {}x, cost: {}, selfish: {}".
        format(numPredators, numPrey, numBlocks, config.maxEpisode, config.maxTimeStep, preySpeedMultiplier,
               costActionRatio, selfishIndex))
    ...
    criticInputDimList = [np.sum(obsShape) + np.sum(policyOutputDimList) for _ in
                          range(numAgents)]  # all agents actions and observations

    maddpg = MADDPG.init_from_env(algorithmTypes, isDiscreteAction, policyInputDimList, policyOutputDimList,
                                  criticInputDimList,
