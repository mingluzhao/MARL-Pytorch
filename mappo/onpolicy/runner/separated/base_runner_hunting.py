...
from onpolicy.utils.separated_buffer_hunting import SeparatedReplayBuffer
...
class Runner(object):
    def __init__(self, config):
        ...
        # env information
        self.all_share_observation_space = config['all_share_observation_space']
        self.all_observation_space = config['all_observation_space']
        self.all_action_space = config['all_action_space']
        ...
        if self.all_args.algorithm_name == "happo":
            ...
            from onpolicy.algorithms.r_mappo.algorithm.rMAPPOPolicy_hunting import R_MAPPOPolicy as Policy

        self.policy = []
        for agent_id in range(self.num_agents):
            share_observation_space = self.all_share_observation_space[agent_id] if self.use_centralized_V else self.all_observation_space[agent_id]
            # policy network
            po = Policy(self.all_args,
                        self.all_observation_space[agent_id],
                        share_observation_space,
                        self.all_action_space[agent_id],
                        device = self.device)
